...
@app.route('/api/inserir-nota', methods=['POST'])
def inserir_nota():
    data = request.get_json()
    ...
    try:
        ...
        """, (
            data.get("cod"),
            data.get("fornecedor"),
            data.get("nome"),        # ou "descricao", depende do seu frontend
            data.get("unid"),
            float(data.get("qtd")) if data.get("qtd") else None,
            float(data.get("vlrUnit")) if data.get("vlrUnit") else None,
            data.get("emissao") or datetime.now().isoformat()
        ))
...
def update_materia(codigo_antigo):
    try:
        ...
        cur.execute("""
            UPDATE historico
            SET descricao_produto = ?,preco_unitario=?, codigo = ?
            WHERE codigo = ?
        """, (novo_nome,preco, novo_codigo, codigo_antigo))
