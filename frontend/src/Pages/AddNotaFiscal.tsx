...
export function AdicionarNotaFiscal() {
  ...
  const handleProdutoChange = async (
    ...
    if (campo === "textoBusca") {
      ...
      const produtoEncontrado = produtosSalvos.find(
      ...
      if (produtoEncontrado) {
        ...
        try {
          const res = await fetch(
            `http://localhost:5000/api/unidades/${produtoEncontrado.codigo}`
          );
    ...
    if (campo === "vlrUnit" || campo === "qtd") {
      const qtd =
        campo === "qtd" ? parseFloat(valor) : parseFloat(novosProdutos[index].qtd);
      const vlr =
        campo === "vlrUnit"
          ? parseFloat(valor)
          : parseFloat(novosProdutos[index].vlrUnit);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const produto of produtos) {
      if (
        ...
        !produto.nome.trim() ||
        !produto.unid.trim() ||
        isNaN(parseFloat(produto.qtd)) ||
        isNaN(parseFloat(produto.vlrUnit))
      ) {
            ...
            <div key={index} className="campo-produto">
              <div className="autocomplete-wrapper">
                <input
                  ...
                  onChange={(e) =>
                    handleProdutoChange(index, "textoBusca", e.target.value)
                  }
                    ...
                    {getSugestoes(produto.textoBusca).map((p, i) => (
                      <div
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch(
                              `http://localhost:5000/api/unidades/${p.codigo}`
                            );
                            const dados = await res.json();
                            novosProdutos[index].unid = dados.unidade || "";
                          } catch (err) {
              ...
              <input
                ...
                placeholder="Unid"
                value={produto.unid ?? ""}
                onChange={(e) => handleProdutoChange(index, "unid", e.target.value)}
              />
